...
import {PlaceCallType} from "../../notifications/types";
...
function HangupButton(props) {
    ...
    return (<AccessibleButton className="mx_RoomView_voipButton"
...
export default createReactClass({
    ...
    _onRoomViewStoreUpdate: function(initial) {
        ...
        // the RoomView instancemx_EventTile
        if (initial) {
    ...
    onHangupClick: function() {
        const call = CallHandler.getCallForRoom(this.state.room.roomId);
        if (!call) {
            return;
        }
        dis.dispatch({
    ...
    onCallPlaced : function (type: PlaceCallType)  {
        dis.dispatch({
            action: 'place_call',
            type: type,
            room_id: this.state.room.roomId,
    ...
    render: function() {
        ...
        if (inCall) {
            ...
            const endCall =
                <div className="mx_RoomView_voipButton mx_EndCall" onClick={this.onHangupClick}>
                <TintableSvg src={require("../../../res/img/voip/hangup.svg")}
                             alt={_t("Hang up")}
                             width="26" height="31" />
            </div>;
