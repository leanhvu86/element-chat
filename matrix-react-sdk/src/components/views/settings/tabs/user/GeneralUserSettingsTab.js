...
export default class GeneralUserSettingsTab extends React.Component {
    ...
    _getBirthdayAndGender(){
        const client = MatrixClientPeg.get();
        client.requestPhoneAndGender().then(result=>{
            console.log(result)
            if(result['date_of_birth']!==undefined){
                this.setState({dateOfBirth: result['date_of_birth']});
            }
    ...
    _renderAccountSection() {
        ...
        const DateOfBirth = sdk.getComponent("views.settings.account.DateOfBirth");
        ...
        if (this.state.haveIdServer || this.state.serverSupportsSeparateAddAndBind === true) {
            ...
            //     />;
            const dateOfBirth = this.state.loading3pids
                ? <Spinner />
                : <DateOfBirth
                ...
                />;
            threepidSection = <div>
                <span className="mx_SettingsTab_subheading">{_t("Birthday")}</span>
                {dateOfBirth}
