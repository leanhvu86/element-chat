...
export default class EditMessageComposer extends React.Component {
    ...
    _onChange = () => {
        if (!this.state.saveDisabled || !this._editorRef || !this._editorRef.isModified()) {
            if(this.state.saveDisabled===false){
                const editedEvent = this.props.editState.getEvent();
                const editContent = createEditContent(this.model, editedEvent);
                const newContent = editContent["m.new_content"];
                this.setState({
                    buttonChoose: !this._isContentModified(newContent),
                });
            }
            return;
        }
    ...
    render() {
        ...
        const cancelButton= <img className="mx_filterFlipColor" src={require("../../../../res/img/cancel-red.svg")} width="12" height="12"
                                 onClick={this._cancelEdit} />
        const sendButton= <img className="mx_filterFlipColor" src={require("../../../../res/img/send.svg")} width="14" height="14"
        ...
        return (<div className={classNames("mx_EditMessageComposer", this.props.className)} onKeyDown={this._onKeyDown}>
            ...
            <div className="mx_EditMessageComposer_buttons">
                <a className="mx_EditMessageComposer_span">{ this.state.buttonChoose ? cancelButton : sendButton }
                </a>
