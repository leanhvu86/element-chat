...
class UploadButton extends React.Component {
    ...
    onUploadFileInputChange(ev) {
        ...
        for (let i = 0; i < ev.target.files.length; ++i) {
            if (this.checkFileUpload(ev.target.files[i]) === false) {
                const ErrorDialog = sdk.getComponent("dialogs.ErrorDialog");
                console.error("File type not allow: ");

                _Modal.default.createTrackedDialog('File type not allowed.', '', ErrorDialog, {
                    title: (0, languageHandler._t)("Error"),
                    description: 'File type not allowed. Please check again.'
                });
                return;
            }
        }
