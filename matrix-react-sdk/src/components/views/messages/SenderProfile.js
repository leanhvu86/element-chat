...
export default createReactClass({
    ...
    render() {
        ...
        const timestamp = mxEvent.getTs() ?
            <span className="mx_EventTile_Timestamp" aria-hidden={true} aria-setsize={8}>
                { formatTime(new Date(mxEvent.getTs()), this.props.showTwelveHour) }
            </span> : null;
        if (msgtype === 'm.emote') {
        ...
        if (this.props.isMe){
            return (
                <div className="mx_SenderProfile_Me" dir="auto" onClick={this.props.onClick}>
                    <div className="mx_MessageTimestamp_layout">
                        {timestamp}
                    </div>
                </div>
            );
