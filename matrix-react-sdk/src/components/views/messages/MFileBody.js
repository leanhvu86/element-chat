...
function updateTintedDownloadImage() {
    ...
    request({uri: require("../../../../res/img/files.svg")}, (err, response, body) => {
        if (err) return;
    ...
    render: function() {
        ...
        if (isEncrypted) {
            ...
            if (this.props.tileShape === "file_grid") {
                ...
                let aStyle={width:'300px',textAlign:'center'}
                return (
                    <span className="mx_MFileBody">
                        <div className="mx_MFileBody_download">
                            <a className="mx_FileBody_info">
                                <img src={tintedDownloadImageURL} width="12" height="14" ref={this._downloadImage} />
                                {/*{ _t("Download %(text)s", { text: text }) }*/}
                                {text}
                            </a>

                            <div className="mx_MFileBody_download_button">
                                <a {...downloadProps}  title={text} style={aStyle}>{_t("Download")}</a>
                            </div>
                        </div>
                    </span>
                );
