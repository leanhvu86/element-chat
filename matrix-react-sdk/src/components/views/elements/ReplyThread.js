...
import {formatTime, wantsDateSeparator} from '../../../DateUtils';
        ...
        switch (ev.getContent().msgtype) {
            ...
            case 'm.notice': {
                html = `<mx-reply><blockquote><a href="${evLink}">In reply to</a> <a href="${userLink}">${mxid}</a>`
                    + `<br><em>${html}</em><server-ts>${ts}</server-ts></blockquote></mx-reply>`;
    ...
    render() {
        ...
        const evTiles = this.state.events.map((ev) => {
            ...
            if (wantsDateSeparator(this.props.parentEv.getDate(), ev.getDate())) {
                let style ={fontSize:'10px',float:'left !important',width:'fit-content !important'}
                const date = new Date(ev.getTs());
                dateSep = <a href={this.props.url} style={style}><DateSeparator ts={ev.getTs()} />{ _t('Send at ')+formatTime(date, false) }</a>;
            }
            let displayName = ev.sender.rawDisplayName;
            let name = <span className="mx_EventTile_avatar_reply_name">{displayName}</span>;
            let avatar = (<div className="mx_EventTile_avatar_reply">
                        <table>
                            <tbody>
                            <tr>
                                <td>
                                    <MemberAvatar member={ev.sender}
                                                  width={10} height={10}
                                                  viewUserOnClick={true}/></td>
                                <td>   {name}</td>
                                <td>  {dateSep}</td>
                            </tr>
                            </tbody>
            ...
            let quote=<div className={"mx_ReplyThread_Quote"}> </div>

            if (ev.event.content.formatted_body!== undefined){
                const n = ev.event.content.formatted_body.indexOf('</mx-reply>');
                let res = ev.event.content.formatted_body.substring(n + 11, ev.event.content.formatted_body.length);
                let content = <div className={"mx_ReplyThread_text"}>{res}</div>
                return <div className="mx_ReplyThread" key={ev.getId()}>
                    {quote}
                    <EventTile
                        mxEvent={ev}
                        tileShape="reply"
                ...
                let res = ev.event.content.body;
                let content = res.split('\n').map(str => <div>{str}</div>);
                content = <div className={"mx_ReplyThread_text"}>{content}</div>
                return  <div className="mx_ReplyThread" key={ev.getId()}>
                    {quote}
                    <EventTile
                        mxEvent={ev}
