...
export default class PasswordReset {
    ...
    async resetPasswordPhone(country,phone) {
        return await this.client.requestPasswordPhoneToken(country,phone, this.clientSecret, 1).then((res) => {
            this.sessionId = res.sid;
            return res;
        }, function(err) {
            if (err.errcode === 'M_THREEPID_NOT_FOUND') {
                err.message = _t('This phone address was not found');
            } else if (err.httpStatus) {
                err.message = err.message + ` (Status ${err.httpStatus})`;
            }
            throw err;
    ...
    async requestChangePasswordNew(phone,otp,new_password,sid) {
        return this.client.requestChangePasswordNew(phone,otp,new_password,sid, this.clientSecret, 1).then((res) => {
            this.sessionId = res.sid;
            return res;
        }, function(err) {
            if (err.errcode === 'M_THREEPID_NOT_FOUND') {
