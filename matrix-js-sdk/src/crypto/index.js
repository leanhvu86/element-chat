...
Crypto.prototype.saveKeysAfterLoginNew = async function(userId,crossSigningStorage) {
    this._deviceList.setRawStoredCrossSigningForUser(
        userId, crossSigningStorage,
    );
}
Crypto.prototype.saveKeysAfterLogin = async function(keys) {
    ...
    const masterKey = keys['master'];
    const encodedKey = olmlib.encodeBase64(masterKey);
    secretStorage.store(`m.cross_signing.master`, encodedKey);
    const userSigning = keys['user-signing'];
    const userSigningEncodedKey = olmlib.encodeBase64(userSigning);
     secretStorage.store(`m.cross_signing.user-signing`, userSigningEncodedKey);
    const selfSigning = keys['self-signing'];
    const selfSigningencodedKey = olmlib.encodeBase64(selfSigning);
     secretStorage.store(`m.cross_signing.self-signing`, selfSigningencodedKey);
};
/**
...
Crypto.prototype.bootstrapSecretStorage = async function({
    ...
    if (setupNewKeyBackup && !keyBackupInfo) {saveKeysAfterLogin
